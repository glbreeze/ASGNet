...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    all_miou = defaultdict(float)
    fb_iou = defaultdict(float)
    for st in [1, 2, 3, 5, 10, 15, 20]:
        logger.info("======================================== test on shot {} ========================================".format(st))
        val_data = dataset.SemData(split=args.split, shot=st, max_sp=args.max_sp, data_root=args.data_root,
        ...
        all_miou[st] = class_miou
        fb_iou[st] = mIoU_val
    logger.info('meanIoU---Val result: mIoU {}.'.format(','.join(f'{key}: {round(value, 4)}' for key, value in all_miou.items())))
    logger.info('FB_IoU----Val result: mIoU {}.'.format(','.join(f'{key}: {round(value, 4)}' for key, value in fb_iou.items())))
...
def validate(val_loader, model, criterion, args):
    ...
    for e in range(20):
        for i, (input, target, s_input, s_mask, s_init_seed, subcls, ori_label) in enumerate(val_loader):
            ...
            if ((i + 1) % (test_num/10) == 0) and main_process():
                logger.info('Test: [{}/{}] '
